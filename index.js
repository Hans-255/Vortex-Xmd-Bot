...
> Simple , Straight Forward But Loaded With Features, 
 Meet *VORTEX XMD* WhatsApp Bot🔥

*Thanks for using VORTEX XMD* 
*Join WhatsApp Channel 🕊️*
> ${config.CHANNEL_LINK || 'https://whatsapp.com/channel/0029Vb7JRfvCRs1gTmsCB812'}

*Join WhatsApp group 😎*
> https://chat.whatsapp.com/${config.GROUP_INVITE_LINK || 'DsoPcRpOcCg73qya4cFeD3'}

*ʏᴏᴜʀ ᴘʀᴇғɪx : ${config.PREFIX}*
────────────────────────────

> *© POWERED BY VORTEX XMD*`
          const startContextInfo = {
            forwardingScore: 5,
            isForwarded: true,
          ...
          const imgUrl = config.MENU_IMAGE_URL
          try {
            await conn.sendMessage(conn.user.id, {
              image: { url: imgUrl },
              caption: startCaption,
              contextInfo: startContextInfo
            })
          } catch (_imgErr) {
            await conn.sendMessage(conn.user.id, {
              text: startCaption,
...
setInterval(async () => {
    if (config.AUTO_BIO === "true") {
        ...
        const bioText = `🛡️ VORTEX BOT 🤖 LIVE NOW\n📅 ${date}\n⏰ ${time}`;
        try {
    ...
    conn.ev.on('messages.upsert', async(mek) => {
      ...
      const rav = ('255753668403', '255741752020')
